...
**Exercício 7:** Renomeie o campo `budget` para `estimateBudget` do filme `Batman`.
...
**Exercício 12:** Utilizando uma única operação, remova os campos `budget` e `estimateBudget` em todos os documentos.
...
As informações acima são sobre a Escola Jean Grey de Ensino Superior. Para todas as alterações relizadas, você deve sempre utilizar ou adicionar o campo **lastUpdate**, que armazena a data da última alteração com o tipo **timeStamp**. Os exercícios devem ser realizados na ordem.
